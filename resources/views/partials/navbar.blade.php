<header class="fixed top-0 right-0 left-0 z-30 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center px-4 lg:px-6" style="left: 240px;">
    <div class="flex items-center flex-1 gap-4">
        <button @click="$dispatch('toggle-sidebar')" class="lg:hidden p-2 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800 transition-all">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
        </button>

        <div class="hidden sm:flex items-center max-w-[320px] w-full">
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="relative">
            <button @click="notifOpen = !notifOpen" @keydown.escape="notifOpen = false"
                    class="relative p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800 transition-all duration-200">
            ...
            <div x-show="notifOpen" @click.away="notifOpen = false"
                 ...
                 class="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden z-50" style="display:none;">
                ...
                <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            @if(auth()->user()->role)
                            <span class="inline-flex items-center mt-1.5 px-2 py-0.5 text-[10px] font-bold rounded-full uppercase tracking-wider bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400">{{ auth()->user()->role->name }}</span>
                            @endif
                        </div>
                    </div>
                </div>
