<header class="navbar h-16 flex items-center justify-between px-4 sm:px-6">
    {{-- Left: Mobile menu + Search --}}
    <div class="flex items-center gap-3">
        <button @click="mobileSidebar = !mobileSidebar" class="lg:hidden p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200 hover:scale-105 active:scale-95">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
        </button>

        {{-- Search with glow effect --}}
        <div class="hidden sm:flex search-wrapper">
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <svg class="w-4 h-4 text-slate-400 transition-colors duration-200" :class="$el.parentElement.parentElement.querySelector('input') === document.activeElement && 'text-primary-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </div>
                <input type="text" placeholder="Search tickets, users..."
                       class="w-64 lg:w-80 pl-10 pr-4 py-2.5 text-sm bg-slate-50/80 border border-slate-200/80 rounded-2xl placeholder-slate-400 transition-all duration-300 focus:outline-none focus:bg-white focus:border-primary-400 focus:shadow-lg focus:shadow-primary-500/5 focus:w-96">
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <kbd class="hidden lg:inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium text-slate-400 bg-slate-100 border border-slate-200 rounded-md">⌘K</kbd>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-1.5">
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" @keydown.escape="open = false"
                    class="relative p-2.5 rounded-2xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200 hover:scale-105 active:scale-95">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                <span class="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-danger-500 rounded-full ring-2 ring-white" style="animation: pulseSoft 2s ease-in-out infinite;"></span>
            </button>
            {{-- Notification dropdown --}}
            <div x-show="open" @click.away="open = false"
                 x-transition:enter="dropdown-enter" x-transition:leave="dropdown-leave"
                 class="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden z-50" style="display:none;">
        ...
        {{-- User dropdown with premium animation --}}
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" @keydown.escape="open = false"
                    class="flex items-center gap-2.5 p-1.5 pr-3 rounded-2xl hover:bg-slate-50 transition-all duration-200 group">
                <div class="w-8 h-8 rounded-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-xs font-bold shadow-sm transition-all duration-200 group-hover:shadow-md group-hover:shadow-primary-500/20 group-hover:scale-105">
            ...
            <div x-show="open" @click.away="open = false"
                 x-transition:enter="dropdown-enter" x-transition:leave="dropdown-leave"
                 class="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl shadow-slate-200/60 border border-slate-100 py-1 z-50 overflow-hidden" style="display:none;">
