...
@if($type === 'text')
    @for($i = 0; $i < $count; $i++)
        <div class="animate-pulse space-y-2 {{ $i > 0 ? 'mt-3' : '' }}">
            <div class="h-4 bg-gray-200 rounded {{ $width ?? 'w-full' }}" {{ $height ? "style='height: $height'" : '' }}></div>
        </div>
    @endfor
@elseif($type === 'heading')
...
@elseif($type === 'table')
    @for($i = 0; $i < $count; $i++)
        <div class="animate-pulse flex items-center space-x-4 py-3 border-b border-gray-100 {{ $i > 0 ? '' : '' }}">
