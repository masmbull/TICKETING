...
<div class="space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Manage ticket categories and subcategories</p>
    </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="card">
                <div class="p-5 border-b border-slate-100">
                    <h2 class="text-base font-bold text-slate-900">New Category</h2>
                </div>
                <div class="p-5">
            ...
            <div class="card">
                <div class="p-5 border-b border-slate-100">
                    <h2 class="text-base font-bold text-slate-900">New Subcategory</h2>
                </div>
                <div class="p-5">
        ...
        <div class="lg:col-span-2">
            <div class="card">
                ...
                <div class="border-b border-slate-100 last:border-b-0">
                    <div class="px-5 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-slate-400 ml-2">{{ $category->slug }}</span>
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-xs text-slate-400">{{ $category->subCategories->count() }} subcategories</span>
                            <span class="inline-flex items-center {{ $category->is_active ? 'bg-success-50 text-success-700' : 'bg-slate-100 text-slate-600' }} text-xs font-bold px-2 py-0.5 rounded-full">
                    ...
                    <div class="bg-slate-50/50 px-5 py-3 ml-6 border-l-2 border-slate-200">
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs text-slate-400">{{ $sub->slug }}</span>
                                </div>
                                <span class="inline-flex items-center {{ $sub->is_active ? 'bg-success-50 text-success-700' : 'bg-slate-100 text-slate-600' }} text-xs font-bold px-2 py-0.5 rounded-full">
                    ...
                    @else
                    <div class="bg-slate-50/50 px-5 py-3 ml-6 border-l-2 border-slate-200">
                        <p class="text-xs text-slate-400 italic">No subcategories yet</p>
                    </div>
                    @endif
                </div>
                @empty
                <div class="p-12 text-center">
                    <svg class="mx-auto h-12 w-12 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 7h.01M7 11h.01M7 15h.01M7 19h.01M4 7h.01M4 11h.01M4 15h.01M4 19h.01"/></svg>
                    <h3 class="mt-4 text-sm font-semibold text-slate-900">No categories yet</h3>
