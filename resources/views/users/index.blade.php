    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-wrap gap-3">
            <form method="GET" class="flex flex-wrap gap-2 flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search users..." class="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-700 border-0 rounded-lg text-slate-900 dark:text-white">
                <select name="role" class="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-700 border-0 rounded-lg text-slate-900 dark:text-white">
                    <option value="">All Roles</option>
                    @foreach($roles as $role)
                    <option value="{{ $role->id }}" {{ request('role') == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                    @endforeach
                </select>
                <select name="department" class="px-3 py-2 text-sm bg-slate-100 dark:bg-slate-700 border-0 rounded-lg text-slate-900 dark:text-white">
                ...
                @if(request()->all())
                <a href="{{ route('users.index') }}" class="px-4 py-2 text-sm text-slate-500 hover:text-slate-700">Clear</a>
                @endif
            </form>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                        ...
                        <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300 hidden lg:table-cell">{{ $user->department?->name ?? '-' }}</td>
                        <td class="px-4 py-3">
                            @if($user->is_active)
                            <span class="px-2 py-1 rounded text-xs font-medium bg-emerald-500/10 text-emerald-600 border border-emerald-200">Active</span>
                            @else
                            <span class="px-2 py-1 rounded text-xs font-medium bg-red-500/10 text-red-600 border border-red-200">Inactive</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('users.show', $user->id) }}" class="p-2 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('users.edit', $user->id) }}" class="p-2 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700">
                                ...
                                <form method="POST" action="{{ route('users.toggle-status', $user->id) }}" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="p-2 rounded-lg {{ $user->is_active ? 'text-orange-500 hover:bg-orange-50' : 'text-emerald-500 hover:bg-emerald-50' }} dark:hover:bg-slate-700">
                                ...
                                <form method="POST" action="{{ route('users.destroy', $user->id) }}" class="inline" x-data="{ show: false }">
                                    @csrf @method('DELETE')
