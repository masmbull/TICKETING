...
<div class="min-h-screen -m-6 p-6 lg:p-8">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
        <div class="stat-card">
            <div class="text-lg font-bold text-slate-900">{{ $tickets->total() }}</div>
            <div class="text-xs text-slate-500 font-medium">Total</div>
        </div>
        <div class="stat-card">
            <div class="text-lg font-bold text-primary-600">{{ $tickets->getCollection()->where('status', 'Open')->count() }}</div>
            <div class="text-xs text-primary-600 font-medium">Open</div>
        </div>
        <div class="stat-card">
            <div class="text-lg font-bold text-warning-600">{{ $tickets->getCollection()->where('status', 'In Progress')->count() }}</div>
            <div class="text-xs text-warning-600 font-medium">In Progress</div>
        </div>
        <div class="stat-card">
            <div class="text-lg font-bold text-success-600">{{ $tickets->getCollection()->where('status', 'Resolved')->count() }}</div>
            <div class="text-xs text-success-600 font-medium">Resolved</div>
        </div>
        <div class="stat-card">
            <div class="text-lg font-bold text-slate-500">{{ $tickets->getCollection()->where('status', 'Closed')->count() }}</div>
    ...
    <div class="card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table">
                <thead>
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Ticket</th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Subject</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="window.location='{{ route('tickets.show', $ticket->id) }}'">
                        <td class="px-4 py-3">
                            <span class="text-sm font-bold text-primary-600">{{ $ticket->ticket_number }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="text-sm font-semibold text-slate-900 max-w-xs truncate">{{ $ticket->subject }}</div>
                            <div class="text-xs text-slate-400 truncate max-w-xs">{{ Str::limit($ticket->description, 50) }}</div>
                        </td>
                        <td class="px-4 py-3">
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center">
                            <div class="empty-state">
                                ...
                                <p class="text-sm font-semibold text-slate-900">No tickets found</p>
