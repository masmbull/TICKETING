...
<div class="space-y-6">
    ...
    <div class="card">
        <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
            <h2 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wide">Timeline</h2>
        </div>
        <div class="px-4 py-3">
            <div class="relative">
                <div class="absolute left-1.5 top-2 bottom-2 w-px bg-slate-200 dark:bg-slate-700"></div>
                <div class="space-y-3">
                    @foreach($timeline as $index => $event)
                    <div class="relative flex items-start gap-3">
                        <div class="relative z-10 flex-shrink-0 mt-0.5">
                            <div class="w-3 h-3 rounded-full border-2 border-white dark:border-slate-800
                        ...
                        <div class="flex-1 min-w-0 pb-3">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <span class="text-xs font-semibold text-slate-900 dark:text-white">{{ $event['label'] }}</span>
                                @if($event['actor'])
                                <span class="text-[10px] text-slate-500 dark:text-slate-400">&middot; {{ $event['actor'] }}</span>
                                @endif
                            </div>
                            <div class="text-[10px] text-slate-400 dark:text-slate-500">{{ $event['timestamp']->format('d M Y, H:i') }}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card">
                <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                    <h2 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wide">Description</h2>
                </div>
                <div class="px-4 py-3">
            ...
            <div class="card">
                <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                    <h2 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wide">Workflow</h2>
                </div>
                <div class="px-4 py-3">
            ...
            <div class="card">
                <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                    <h2 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wide">Support Notes</h2>
                </div>
            ...
            <div class="card">
                <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                    <h2 class="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wide">Comments ({{ $ticket->comments->count() }})</h2>
                </div>
                <div class="divide-y divide-slate-50 dark:divide-slate-800">
                    @forelse($ticket->comments as $comment)
                    <div class="px-4 py-3">
                        <div class="flex items-start gap-2.5">
                            <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                                {{ strtoupper(substr($comment->user->name, 0, 2)) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1.5 mb-0.5">
                                    <span class="text-xs font-semibold text-slate-900 dark:text-white">{{ $comment->user->name }}</span>
                                    @if($comment->user->role)
                                    <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wide bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">{{ $comment->user->role->name }}</span>
                                    @endif
                                    <span class="text-[10px] text-slate-400 dark:text-slate-500">{{ $comment->created_at->diffForHumans() }}</span>
                                </div>
                                <p class="text-xs text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{{ $comment->comment }}</p>
                            </div>
                        </div>
                    </div>
                    @empty
                ...
                {{-- Comment Form --}}
                <div class="px-4 py-3 border-t border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/50">
                    <form method="POST" action="{{ route('tickets.comments.store', $ticket->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="flex items-start gap-2.5">
                            ...
                            <div class="flex-1">
                                <textarea name="comment" rows="2" required class="input" placeholder="Add a comment...">{{ old('comment') }}</textarea>
