...
<div class="min-h-screen -m-6 p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('tickets.index') }}" class="p-2 text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800 rounded-lg hover:bg-slate-100 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-bold text-slate-900 dark:text-white">{{ $ticket->ticket_number }}</h1>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ $ticket->subject }}</p>
            </div>
        </div>
                ...
                <div class="px-5 py-3 border-t border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/50">
                    <div class="flex flex-wrap gap-2">
                        ...
                        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm dark:bg-slate-800 dark:border-slate-700">
                            <svg class="w-4 h-4 text-slate-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>
                            <span class="text-slate-700 dark:text-slate-300">{{ $attachment->original_filename }}</span>
                            <span class="text-slate-400 dark:text-slate-500 text-xs">({{ round($attachment->file_size / 1024, 1) }} KB)</span>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="px-5 py-4">
                    <dl class="space-y-4 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $ticket->category->name ?? '-' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-slate-500 dark:text-slate-400">Sub Category</dt>
                            <dd class="font-semibold text-slate-900 dark:text-white">{{ $ticket->subCategory->name ?? '-' }}</dd>
                        </div>
                        @if($ticket->assignee)
                        <div class="flex justify-between">
                            <dt class="text-slate-500 dark:text-slate-400">Assignee</dt>
                            <dd class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-lg bg-primary-100 text-primary-700 dark:bg-primary-500/20 dark:text-primary-300 flex items-center justify-center text-[10px] font-bold">{{ strtoupper(substr($ticket->assignee->name, 0, 1)) }}</div>
                                <span class="font-semibold text-slate-900 dark:text-white">{{ $ticket->assignee->name }}</span>
                            </dd>
                        </div>
                        @endif
                    </dl>
                ...
                <div class="px-5 py-4">
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-slate-700 dark:text-slate-300 font-medium">Resolved</div>
                                <div class="text-xs text-slate-400 dark:text-slate-500">{{ $ticket->resolved_at->format('d M Y, H:i') }}</div>
                            </div>
                        </div>
                        @endif
                        @if($ticket->closed_at)
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="text-slate-700 dark:text-slate-300 font-medium">Closed</div>
                                <div class="text-xs text-slate-400 dark:text-slate-500">{{ $ticket->closed_at->format('d M Y, H:i') }}</div>
                            </div>
                        </div>
                        @endif
                    </div>
